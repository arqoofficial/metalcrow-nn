...
def get_environment(sentence, materials_):
    ...
    env_materials.extend(
        {"text": t.text, "tok_ids": [t.i]} for t in sentence if t.text in atmospheres
    )
...
def __get_tok_values(sentence_tokens, token):
    ...
    if (
        sentence_tokens[temp_id - 1] in [",", "and", "to", "or"]
        and "-" not in sentence_tokens[temp_id]
    ):
    ...
    token_num = re.sub(
        "([0-9\.\,\-]*)\s*[°A-Za-z]*", "\\1", sentence_tokens[temp_id]
    ).replace(",", "")
    ...
    if sentence_tokens[temp_id - 1] == "°" and value_1 == "":
        value = re.sub(
            "([0-9\.\,\-]*)\s*[°A-Za-z]*", "\\1", sentence_tokens[temp_id - 2]
        ).replace(", ", "")
        return {
            "max": float(value_2),
            "min": float(value),
