...
def test_markdown_sets_resolution_headers(api_client, shared_root: Path) -> None:
    ...
    assert decode_path_header(response.headers["X-Resolved-Path"]) == raw_to_stage0_okf(raw)
...
def test_markdown_supports_non_ascii_paths(api_client, shared_root: Path) -> None:
    ...
    assert decode_path_header(response.headers["X-Resolved-Path"]) == raw_to_stage1_okf(raw)
