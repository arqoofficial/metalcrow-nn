...
from app.presentation.http_headers import encode_path_header
...
from app.services.list_service import list_raw_files
...
def files_tree(
    ...
    limit: int = Query(10, description="Max direct children returned at resolved root (1–1000)."),
) -> FileTreeResponse:
    try:
...
def markdown_content(
    ...
    return PlainTextResponse(
        ...
        headers={
            "X-Requested-Path": encode_path_header(okf_path),
            "X-Resolved-Path": encode_path_header(resolved_okf),
        },
    )
...
@router.get(
    ...
    description=(
        "Returns the original raw document bytes for an exact path under `SHARED/`. "
        "Logical paths resolve via UPLOAD_DATA then RAW_DATA."
    ),
    responses={
        400: _COMMON_ERRORS[400],
        404: _COMMON_ERRORS[404],
...
def raw_file_content(
    ...
    path: str = Query(
        ...,
        description="Logical or concrete raw path, e.g. `reports/q1.pdf` or `UPLOAD_DATA/reports/q1.pdf`.",
    ),
) -> FileResponse:
    config = _config(request)
    try:
        resolved = resolve_exact_raw_path(path, config.shared_root)
    except PathValidationError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if resolved is None:
        raise HTTPException(status_code=404, detail="raw file not found")

    target = Path(config.shared_root) / resolved.relative
    return FileResponse(
        path=target,
        filename=Path(resolved.relative).name,
        headers={
            "X-Requested-Path": encode_path_header(path),
            "X-Resolved-Path": encode_path_header(resolved.relative),
        },
    )
...
@router.get(
    "/files/list",
    response_model=RawFileListResponse,
    tags=[TAG_BROWSE],
    summary="List raw files as a flat catalog",
    description=(
        "Returns a paginated flat list of concrete raw files under a source prefix "
        "(`RAW_DATA` by default). Supports case-insensitive substring search on the full path. "
        "With `unparsed_only=true` (default), files that already have stage-0 OKF are excluded."
    ),
...
def files_list(
    ...
    source: str = Query(
        "RAW_DATA",
        description="Source folder under `SHARED/`: `RAW_DATA` or `UPLOAD_DATA`.",
    ),
    search: str = Query("", description="Case-insensitive substring filter on the full path."),
    extension: str = Query(".pdf", description="File extension filter, e.g. `.pdf`."),
    unparsed_only: bool = Query(
        True,
        description="When `true`, exclude files that already have stage-0 OKF on disk.",
    ),
    offset: int = Query(0, ge=0, description="Pagination offset (≥ 0)."),
    limit: int = Query(10, ge=1, le=1000, description="Page size (1–1000)."),
) -> RawFileListResponse:
    if source not in {"RAW_DATA", "UPLOAD_DATA"}:
        raise HTTPException(status_code=400, detail="source must be RAW_DATA or UPLOAD_DATA")
    items, total = list_raw_files(
        _config(request).shared_root,
        source=source,
        search=search,
        extension=extension,
    ...
    return RawFileListResponse(data=items, count=total, offset=offset, limit=limit)
...
def reindex(request: Request, body: ReindexRequest) -> ReindexResponse:
    count = enqueue_reindex(_config(request), _stage0_queue(request), enforce=body.enforce)
    return ReindexResponse(enqueued=count)
