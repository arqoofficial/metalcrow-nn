...
class RawFileListItem(BaseModel):
    path: str = Field(..., description="Concrete raw path relative to `SHARED/`.")
    filename: str = Field(..., description="Basename of the raw file.")
    stage0_done: bool = Field(
        ...,
        description="`true` when stage-0 OKF (`00_docling_raw/...`) exists on disk.",
    )
    stage1_done: bool = Field(
        ...,
        description="`true` when stage-1 OKF (`01_docling_clean00/...`) exists on disk.",
    )


class RawFileListResponse(BaseModel):
    data: list[RawFileListItem]
    count: int = Field(..., description="Total matches before pagination.")
    offset: int
    limit: int
