...
def test_stage0_skips_existing_output_when_not_enforced(shared_root: Path) -> None:
    config = make_config(shared_root)
    seed_raw(shared_root, "UPLOAD_DATA/reports/q1__v01.pdf")
    with patch.object(stage0_module, "convert_raw_to_markdown", return_value="# doc"):
        first = run_stage0_job(config, _job())
    assert first is not None
    second = run_stage0_job(config, _job())
    assert second is not None
    assert second.stage == QueueStage.docling_raw2docling_clean00
