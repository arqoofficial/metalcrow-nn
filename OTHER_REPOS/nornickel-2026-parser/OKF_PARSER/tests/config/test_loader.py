...
def test_shared_root_relative_to_config_file_directory(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    project_root = tmp_path / "OKF_PARSER"
    config_dir = project_root / "config"
    shared_root = tmp_path / "SHARED"
    config_dir.mkdir(parents=True)
    shared_root.mkdir()
    config_path = _write_config(config_dir, _valid_config_dict("../../SHARED"))
    other_cwd = tmp_path / "other_cwd"
    other_cwd.mkdir()
    monkeypatch.chdir(other_cwd)

    config = load_config(config_path, config_dir / "missing.env")

    assert config.shared_root == str(shared_root.resolve())
