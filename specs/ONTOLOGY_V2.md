...
> **Код = источник правды:** [`services/ontology-knowledge-graph/ontology/contracts.py`](../services/ontology-knowledge-graph/ontology/contracts.py) —
> pydantic-схема (её импортируют экстракторы, ETL, API, фронтенд);
> [`services/ontology-knowledge-graph/ontology/ddl.sql`](../services/ontology-knowledge-graph/ontology/ddl.sql) — та же схема в Postgres;
> [`services/ontology-knowledge-graph/ontology/demo.py`](../services/ontology-knowledge-graph/ontology/demo.py) — исполняемый скелет.
> **Реализация пакета** (хранилище, загрузчик, конвейер экстракции, query-слой,
> роутер, тул-сервис, тесты): [`services/ontology-knowledge-graph/README.md`](../services/ontology-knowledge-graph/README.md).
> **Запуск демо:** `python -m ontology.demo` из `services/ontology-knowledge-graph/`
> (зависимость — pydantic).
>
...
**Канон** = нормализованные таблицы `experiments.*` (полный DDL — `services/ontology-knowledge-graph/ontology/ddl.sql`).
