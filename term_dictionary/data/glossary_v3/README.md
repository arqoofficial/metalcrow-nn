...
## E2 extraction benchmark (proves the 21× translates to extraction)

Dropped into the E2 head (spaCy `ru_core_news_lg` + EntityRuler), same silver
gold / slice / matching as v2 — only the gazetteer changed:
...
| **EQUIPMENT F1** | **0.138** | **0.508** | **3.7×** |

EQUIPMENT F1 0.14 → 0.51 now *matches NuExtract-3shot's own* EQUIPMENT F1 — the
extractor's competence distilled into a free CPU lookup. Lift is recall-driven
(precision dips as predicted below); `MIN_MENTIONS`/`AGREEMENT` are the precision
levers. Full breakdown + repro: [`eval/BENCHMARK_E2.md`](eval/BENCHMARK_E2.md).
